...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("podcast_watcher.log")],
)
...
def load_state(state_file):
    ...
    try:
        ...
        logger.warning(
            f"Could not parse state file {state_file}, starting with empty state"
        )
        return {"feeds": {}, "last_check": None}
...
def get_feed_episodes(feed_url):
    ...
    try:
        ...
        if parsed_feed.bozo and hasattr(parsed_feed, "bozo_exception"):
            logger.warning(
                f"Feed parsing error for {feed_url}: {parsed_feed.bozo_exception}"
            )
...
def get_audio_url(episode):
    ...
    if hasattr(episode, "enclosures") and episode.enclosures:
        for enclosure in episode.enclosures:
            if (
                "url" in enclosure
                and "type" in enclosure
                and enclosure["type"].startswith("audio/")
            ):
    ...
    if hasattr(episode, "media_content") and episode.media_content:
        for media in episode.media_content:
            if (
                "url" in media
                and "type" in media
                and media["type"].startswith("audio/")
            ):
                return media["url"]
...
def transcribe_episode(
    ...
    try:
        ...
        for file in os.listdir(md_dir):
            if file.endswith((".jpg", ".jpeg", ".png", ".gif")) and os.path.isfile(
                ...
                logger.info(
                    f"Moved image {file} to images directory and updated references"
                )
...
def process_episode(feed_info, episode, config, temp_dir, state):
    ...
    if (
        feed_url in state["feeds"]
        and episode_id in state["feeds"][feed_url]["processed_episodes"]
    ):
...
def process_feed(feed_info, state, config, initial_limit=None):
    ...
    if initial_limit and initial_limit.lower() != "all":
        try:
            ...
            logger.warning(
                f"Invalid initial limit '{initial_limit}', processing all episodes"
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for episode in episodes:
                future = executor.submit(
                    process_episode, feed_info, episode, config, temp_dir, state
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Watch OPML file for new podcast episodes and transcribe them"
    )
    parser.add_argument(
        "opml_file", help="Path to the OPML file containing podcast feeds"
    )
    parser.add_argument(
        "--config",
        default="podcast_watcher_config.json",
        help="Path to configuration file",
    )
    parser.add_argument(
        "--initial",
        default=None,
        help="Number of existing episodes to transcribe on first run (number or 'all')",
    )
    parser.add_argument(
        "--state-file", default=None, help="Path to state file (overrides config file)"
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Root directory for output (overrides config file)",
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="Run once and exit (don't continue watching feeds)",
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        "--podcast2md", default=None, help="Path to podcast2md executable"
    )
    parser.add_argument(
        "--model",
        default=None,
    ...
    parser.add_argument(
        "--skip-formatting", action="store_true", help="Skip applying text formatting"
    )
    parser.add_argument(
        "--skip-links", action="store_true", help="Skip creating wiki links"
    )
    parser.add_argument(
        "--skip-refs", action="store_true", help="Skip adding references"
    )
    ...
    try:
        ...
        logger.info(
            f"Completed processing {len(feeds)} feeds, {total_processed} new episodes transcribed"
        )
        ...
        logger.info(
            f"Continuing to watch feeds, checking every {config['check_interval']} seconds"
        )
